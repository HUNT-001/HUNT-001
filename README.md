<p align="center">
  <img src="assets/hero-name.svg" alt="Tanush Pavan" width="100%" />
</p>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=20&pause=1500&color=93C5FD&center=true&vCenter=true&width=980&lines=World+models+%E2%80%A2+RL+%E2%80%A2+GNNs+%E2%80%A2+Agentic+AI;RTL+design+%E2%80%A2+verification+%E2%80%A2+AI+accelerators;Quantum-curious%2C+silicon-committed;Research-driven+ideas%2C+built+for+real+engineering" alt="Typing SVG" />
</p>
...
<p align="center">
  <b>I build intelligent systems at both ends of the stack &mdash; the algorithm that decides, and the silicon it decides on.</b>
</p>
...
<p align="center">
  <img src="assets/terminal-boot.svg" alt="Boot Sequence" width="100%" />
</p>
...
<p align="center">
I'm a dual-degree student working on <b>efficient intelligent systems</b> &mdash; the kind that have to survive<br>
...
<p align="center">
🧠 <b>Above the line</b> &mdash; world models, model-based RL, graph neural networks, agentic systems, quantum-inspired ML<br>
⚙️ <b>Below the line</b> &mdash; RTL design, functional verification, AI accelerators, quantized inference, edge deployment<br>
🔬 <b>Across it</b> &mdash; hardware-aware ML, HW/SW co-design, and my own algorithmic research
</p>

<details>
<summary><b>&nbsp;Profile summary &mdash; click to expand</b></summary>
...
<p align="center">
  <img src="assets/research-radar.svg" alt="Research Surface" width="100%" />
</p>

<p align="center">
<i>Six threads, one question: how do you build something that models its world well enough to act in it &mdash; cheaply enough to matter?</i>
</p>
...
<p align="center">
  <img src="assets/eq-worldmodel.svg" alt="RSSM Variational Objective" width="100%" />
</p>

<p align="center">
<i>The formalism I actually work in. Not decoration &mdash; these are the objects I'm debugging when something doesn't converge.</i>
</p>

<details open>
<summary><b>&nbsp;🌍 &nbsp;World Models &mdash; learning a latent you can plan inside</b></summary>
...
Training maximises the evidence lower bound &mdash; reconstruct the observation, predict the reward, and pay a KL price for every bit of surprise smuggled into the latent:
...
**The part that's genuinely hard:** $\beta$ is the whole argument. Too low and the model memorises pixels instead of learning consequence. Too high and the posterior collapses onto the prior &mdash; the model stops dreaming, and every rollout returns the same beige future.

</details>

<details>
<summary><b>&nbsp;🎯 &nbsp;Reinforcement Learning &mdash; the objects underneath</b></summary>
...
**Bellman optimality** &mdash; the fixed point every value-based method is chasing:

$$Q^{*}(s,a) \;=\; \mathbb{E}_{s' \sim P}\!\left[\, r + \gamma \max_{a'} Q^{*}(s', a') \;\middle|\; s, a \,\right]$$

**Policy gradient** &mdash; when the action space stops being something you can argmax over:

$$\nabla_\theta J(\theta) \;=\; \mathbb{E}_{\pi_\theta}\!\left[\, \nabla_\theta \ln \pi_\theta(a_t \mid s_t)\, A^{\pi}(s_t, a_t) \,\right]$$

**GAE** &mdash; the bias/variance dial on the advantage estimate:

$$\hat{A}^{\mathrm{GAE}(\gamma,\lambda)}_t \;=\; \sum_{l=0}^{\infty} (\gamma\lambda)^{l}\, \delta_{t+l}, \qquad \delta_t = r_t + \gamma V(s_{t+1}) - V(s_t)$$

**Why model-based:** model-free RL pays for every gradient step in real environment interactions. A world model converts sample complexity into compute complexity &mdash; and compute is the thing I know how to make cheaper in hardware.

</details>

<details>
<summary><b>&nbsp;🕸️ &nbsp;Graph Neural Networks &mdash; because a netlist is a graph</b></summary>

<br>

Message passing in its general form &mdash; aggregate from the neighbourhood, update, repeat:
...
<details>
<summary><b>&nbsp;🤖 &nbsp;Agentic AI &mdash; planning with a budget</b></summary>
...
**Search over reasoning** &mdash; tree search with a learned value, borrowed wholesale from planning:

$$\mathrm{UCT}(s,a) \;=\; Q(s,a) \;+\; c\,\sqrt{\frac{\ln N(s)}{N(s,a)}}$$

**Where I've shipped this:** an agentic AI system for fintech, built at Mumbai Hacks 2024 &mdash; finished **runner-up**. Constrained action space, real cost model, hard correctness requirements. Financial agents are a good forcing function: they make you take $c(a)$ seriously.

</details>

<details>
<summary><b>&nbsp;⚡ &nbsp;Quantized &amp; Binary Networks &mdash; the math that makes edge inference possible</b></summary>

<br>

Uniform affine quantization &mdash; scale $s$, zero-point $z$, round-to-nearest:

$$\hat{x} \;=\; s\left(\mathrm{clip}\!\left(\left\lfloor \frac{x}{s} \right\rceil + z,\; q_{\min},\; q_{\max}\right) - z\right)$$

Rounding has zero gradient almost everywhere, so training uses the straight-through estimator &mdash; pretend the quantizer is the identity inside the clipping range:
...
which turns a multiply-accumulate into `XNOR` + `popcount` &mdash; and *that* is a sentence about hardware, not about machine learning.

**The constraint that decides everything** &mdash; arithmetic intensity against the roofline:
...
<!-- ═══════════════════════════════ QUANTUM ═══════════════════════════════ -->
<img src="assets/banner-quantum.svg" alt="Quantum" width="100%" />

<p align="center">
  <img src="assets/eq-quantum.svg" alt="Variational Quantum Eigensolver" width="100%" />
</p>

<details>
<summary><b>&nbsp;⚛️ &nbsp;The quantum thread &mdash; what I'm actually studying</b></summary>
...
Gradients come from the **parameter-shift rule** &mdash; exact, not finite-difference, which is the detail that makes the whole thing trainable:

$$\frac{\partial E}{\partial \theta_i} \;=\; \frac{1}{2}\left[ E\!\left(\theta_i + \tfrac{\pi}{2}\right) - E\!\left(\theta_i - \tfrac{\pi}{2}\right) \right]$$

**The honest caveat:** barren plateaus. For a random deep ansatz on $n$ qubits, gradient variance decays as $\mathcal{O}(2^{-n})$ &mdash; the landscape flattens exponentially and the optimiser has nothing to descend. Structured ansätze and local cost functions are the live area, and I'm reading rather than claiming here.
...
<p align="center">
  <img src="assets/silicon-flow.svg" alt="RTL to GDSII flow" width="100%" />
</p>

<details open>
<summary><b>&nbsp;⏱️ &nbsp;Timing, power, area &mdash; the three numbers you are always trading</b></summary>

<br>

**Setup closure** &mdash; the clock period has to cover the whole combinational path:

$$T_{\text{clk}} \;\ge\; t_{cq} \;+\; t_{\text{logic,max}} \;+\; t_{\text{setup}} \;-\; t_{\text{skew}} \;+\; t_{\text{jitter}}$$

**Hold closure** &mdash; the failure mode that survives simulation and kills silicon, because it is frequency-independent:
...
The quadratic on $V_{DD}$ is the single most exploitable fact in low-power design &mdash; and the reason DVFS beats almost any microarchitectural trick you can name.

**Amdahl, for accelerator scoping** &mdash; the sentence that kills bad accelerator proposals early:
...
**Concretely:** at ADRIN (ISRO) I designed a custom HLS-based datapath to deploy **MobileViTv2** &mdash; a vision transformer &mdash; on the **AMD Versal ACAP VCK-190**. Transformers on edge silicon are an exercise in memory hierarchy, not in FLOPs. The attention block is the bandwidth problem; everything else is arithmetic you can schedule.
...
<p align="center">
  <img src="assets/verification-stack.svg" alt="Verification methodology" width="100%" />
</p>

<details>
<summary><b>&nbsp;🔍 &nbsp;Why constrained-random needs so many runs &mdash; and when to stop</b></summary>
...
For $n = 1000$ bins that is roughly **7,500 runs** &mdash; and the tail is worse than the mean suggests. This is the quantitative argument for *directed* tests on the hard corners: you do not random-walk into a 1-in-$10^6$ state, you go there deliberately.
...
<details>
<summary><b>&nbsp;🐍 &nbsp;cocotb &mdash; and the open-source tooling around it</b></summary>

<br>

Python testbenches are not a toy. They give you the whole scientific stack next to your DUT: numpy for reference models, pytest for structure, CI that actually runs on every push. The tradeoff is simulation speed at the boundary &mdash; which matters less than people assume for anything below full-chip.
...
<!-- ═══════════════════════════════ FABRICATION ═══════════════════════════════ -->
<img src="assets/banner-fabrication.svg" alt="Fabrication" width="100%" />

<p align="center">
  <img src="assets/fab-stack.svg" alt="Fabrication cross-section" width="100%" />
</p>

<details>
<summary><b>&nbsp;🔬 &nbsp;Process physics &mdash; the constraints that reach all the way up to RTL</b></summary>
...
EUV at $\lambda = 13.5\,\text{nm}$ with $\mathrm{NA} = 0.33$ gets you to roughly 13 nm half-pitch in a single exposure. Below that you either multi-pattern (LELE, SADP, SAQP &mdash; each adding cost, mask count and overlay error) or you move to High-NA at $0.55$ and accept a smaller field. Note the square in the DOF term: every gain in resolution costs you focus budget quadratically. There is no free tightening.
...
**Why an RTL person should know this:** wire delay does not scale like gate delay. Beyond a certain node, interconnect dominates &mdash; which means floorplan is a microarchitectural decision, not a backend one, and locality in your dataflow is worth more than gate count.
...
2. **Find the constraint that actually binds.** Usually memory bandwidth, sample complexity, or verification effort &mdash; rarely the thing the paper emphasises.
...
<p align="center">
  <img src="assets/achievements-wall.svg" alt="Competition record" width="100%" />
</p>

| Competition | Result | Domain |
|:--|:--|:--|
| **Smart India Hackathon 2025** | 🏅 Grand Finale &mdash; national finalist | Govt. of India, national-scale problem statement |
...
<p align="center">
<i>Four national-level finals across three unrelated domains &mdash; agentic AI, analog design, and system architecture.<br>
...
The page is the portfolio piece &mdash; so here is the design system it runs on.</i>
</p>
...
**Colour tokens** &mdash; a single ramp from substrate to signal, with a violet axis reserved for anything quantum or probabilistic:
...
**Type scale** &mdash; three families, three jobs. Segoe UI for headings (authority), JetBrains Mono / Consolas for anything machine-adjacent (labels, code, annotation), and a math serif for equations. Section headers run at 30px / 700 / +10 letter-spacing; annotations at 9–11px mono with lowered opacity so they read as marginalia rather than content.

**Motion, deliberately restrained** &mdash; every animation is a 3–9 second loop with no easing spikes, so nothing competes for attention. Travelling pulses along circuit rails, a slow scan sweep across banners, coverage bars that fill once and freeze, a Bloch vector that precesses. Motion signals *aliveness*, not urgency.

**Layout rules** &mdash; a fixed 1400px design width with 16–18px corner radii, consistent 60px side gutters, and a dot-grid at 10% opacity across every panel so the whole page reads as one continuous surface rather than a stack of unrelated images.

**Accessibility notes** &mdash; every graphic carries `alt` text; nothing critical is conveyed by colour alone; contrast ratios on body text clear 7:1 against the substrate; the page degrades to readable structured markdown if images fail to load.
...
- **Hand-authored SVG** for every banner, divider, diagram and equation plate &mdash; generated from parameterised Python so the geometry is computed, not eyeballed
...
- **Progressive disclosure** via `<details>` &mdash; the page skims in thirty seconds and reads for thirty minutes, depending on what you came for
...
<p align="center">
  <img src="assets/tile-systemverilog.svg" height="53" />
  <img src="assets/tile-rtl.svg" height="53" />
  <img src="assets/tile-soc.svg" height="53" />
  <img src="assets/tile-digital-ic.svg" height="53" />
  <img src="assets/tile-uvm.svg" height="53" />
  <img src="assets/tile-cocotb.svg" height="53" />
</p>

<h3 align="center">EDA / FPGA / Simulation</h3>

<p align="center">
  <img src="assets/tile-vivado.svg" height="53" />
  <img src="assets/tile-ltspice.svg" height="53" />
  <img src="https://skillicons.dev/icons?i=matlab" height="70" />
</p>

<h3 align="center">Machine Learning / AI</h3>

<p align="center">
  <img src="https://skillicons.dev/icons?i=pytorch,tensorflow" height="70" />
  <img src="assets/tile-agentic-ai.svg" height="53" />
  <img src="assets/tile-deep-learning.svg" height="53" />
  <img src="assets/tile-machine-learning.svg" height="53" />
</p>

<h3 align="center">Programming</h3>

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,c,java" height="70" />
  <img src="assets/tile-embedded-c.svg" height="53" />
</p>

<h3 align="center">Data / Visualization</h3>

<p align="center">
  <img src="https://skillicons.dev/icons?i=r" height="70" />
  <img src="assets/tile-tableau.svg" height="53" />
</p>

<h3 align="center">Cloud / Version Control</h3>

<p align="center">
  <img src="assets/tile-git.svg" height="53" />
...
<p align="center">
  <img src="assets/certifications-panel.svg" alt="Certifications and coursework" width="100%" />
</p>

| Credential | Issuer | Why it's here |
|:--|:--|:--|
| [Hardware Security](https://www.coursera.org/learn/hardware-security) | University of Maryland | trojans, side channels, and PUFs &mdash; the attacks that live *below* the software threat model |
| VLSI Design | L&T EduTech | industry-framed digital design flow, end to end |
| [Algorithms Specialization](https://www.coursera.org/specializations/algorithms) | Stanford University | the four-course track &mdash; divide & conquer, graphs, greedy/DP, NP-completeness |
...
<p align="center">
<i>Hardware Security and the Algorithms specialization are the two that show up most in my actual work &mdash;<br>
...
<p align="center">
  <b>BS in Data Science</b> &mdash; Indian Institute of Technology Madras<br>
  <b>B.Tech in Electrical &amp; Electronics Engineering</b> &mdash; Amrita Vishwa Vidyapeetham
</p>
